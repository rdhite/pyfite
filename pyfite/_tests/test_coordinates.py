...
def forward_reverse(
    crs1: pfc.CoordinateReferenceSystem,
    crs2: pfc.CoordinateReferenceSystem,  # pylint: disable=too-many-arguments
    pts1: np.ndarray,
    pts2: np.ndarray,
    tol1: float,
    tol2: float,
    ...
    np.testing.assert_array_almost_equal(forward_actual, forward_expected, tol1)

    reverse_expected = pts1
    reverse_actual = reverse(pts2)
    np.testing.assert_array_almost_equal(reverse_actual, reverse_expected, tol2)
...
def test_gdc_get_proj_str():
    ...
    # no offset
    crs = pfc.Geodetic()
    assert_proj_str_equivalent(crs.get_proj_str(), "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")

    # offset is arbitrary as it does not matter here
    crs = pfc.Geodetic(offset=(np.random.rand(), np.random.rand(), np.random.rand()))
    assert_proj_str_equivalent(crs.get_proj_str(), "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")
...
def test_gcc_get_proj_str():
    ...
    # no offset
    crs = pfc.Geocentric()
    assert_proj_str_equivalent(crs.get_proj_str(), "+proj=geocent +ellps=WGS84")

    # offset is arbitrary as it does not matter here
    crs = pfc.Geocentric(offset=(np.random.rand(), np.random.rand(), np.random.rand()))
    assert_proj_str_equivalent(crs.get_proj_str(), "+proj=geocent +ellps=WGS84")
...
def test_utm_get_proj_str():
    ...
    for zone, south in [
        ...
        crs_off = pfc.Utm(zone, south, offset=(np.random.rand(), np.random.rand(), np.random.rand()))
...
def test_proj_get_proj_str():
    ...
    for proj in projs:
        ...
        assert_proj_str_equivalent(
            pfc.ProjCrs(proj, offset=(np.random.rand(), np.random.rand(), np.random.rand())).get_proj_str(),
            proj,
        )
...
def test_offset_application():
    ...
    gcc_pts = pfc.CoordinateConverter(pfc.Geodetic(), pfc.Geocentric())(gdc_pts)
    utm_pts = pfc.CoordinateConverter(pfc.Geodetic(), pfc.Utm(zone=18, south=False))(gdc_pts)
    ...
    np.testing.assert_array_almost_equal(off_gcc_pts_actual, off_gcc_pts_expected, __METER_TOLERANCE)
...
def test_ltp_gdc():
    ...
    np.testing.assert_array_almost_equal(gdc_actual[:, :2], gdc_expected[:, :2], __DEGREE_TOLERANCE)  # degree parts
    np.testing.assert_array_almost_equal(gdc_actual[:, 2], gdc_expected[:, 2], __METER_TOLERANCE)  # meter part

    ltp_actual = pfc.CoordinateConverter(gdc, ltp)(gdc_expected)
    np.testing.assert_array_almost_equal(ltp_actual, ltp_expected, __METER_TOLERANCE)
...
def test_ltp_gcc():
    ...
    forward_reverse(ltp, gcc, ltp_expected, gcc_expected, __METER_TOLERANCE, __METER_TOLERANCE)
...
def test_ltp_utm():
    ...
    forward_reverse(ltp, utm, ltp_expected, utm_expected, __METER_TOLERANCE, __METER_TOLERANCE)
...
def test_gdc_gcc():
    ...
    np.testing.assert_array_almost_equal(gcc_actual, gcc_actual, __METER_TOLERANCE)
...
def test_gdc_utm():
    ...
    np.testing.assert_array_almost_equal(utm_actual, utm_actual, __METER_TOLERANCE)
...
def test_gcc_utm():
    ...
    forward_reverse(gcc, utm, gcc_expected, utm_expected, __METER_TOLERANCE, __METER_TOLERANCE)
