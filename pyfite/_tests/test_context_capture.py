...
def test_context_capture_regex():
    ...
    assert_proj_str_equivalent(str(metadata_enu.get_crs()), "ENU -121.4953 34.123 0.0")
    assert_proj_str_equivalent(str(metadata_enu_offset.get_crs()), "ENU -100.4124 50.124 0.0 10.0 15.0 0.0")
    assert_proj_str_equivalent(
        str(metadata_epsg.get_crs()),
        "+proj=utm +zone=30 +south +datum=WGS84 +units=m +no_defs +type=crs",
    )
    assert_proj_str_equivalent(
        str(metadata_epsg_offset.get_crs()),
        "+proj=utm +zone=50 +datum=WGS84 +units=m +no_defs +type=crs 23425.0 623423.0 0.0",
    )
    assert_proj_str_equivalent(
        str(metadata_epsg4326.get_crs()),
        "+proj=longlat +datum=WGS84 +no_defs +type=crs",
    )
    assert_proj_str_equivalent(
        str(metadata_epsg4326_offset.get_crs()),
