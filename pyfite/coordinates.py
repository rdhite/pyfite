...
class CrsDefError(Exception):
    ...
    def __init__(
        self,
        message: str = "Could not parse provided string representation of coordinate reference system",
...
class BadCrsError(Exception):
    ...
    def __init__(
        self, message: str = "A required CRS was not valid for the operation"
    ):  # pylint: disable=useless-super-delegation
        super().__init__(message)
...
class CoordinateReferenceSystem(ABC):
    ...
    def find_str(string: str) -> str:
        ...
        pattern = re.compile(
            f"(?:{LocalTangentPlane._srepRegex.pattern})|"  # pylint: disable=protected-access
...
class LocalTangentPlane(CoordinateReferenceSystem):
    ...
    _srepRegex = re.compile(
        f"(?:ltp|enu) ({DECIMAL_REGEX}) ({DECIMAL_REGEX}) ({DECIMAL_REGEX}){_OPTIONAL_OFFSET}",
        re.IGNORECASE,
    )
...
class ProjCrs(CoordinateReferenceSystem):
    ...
    _srepRegex = re.compile(
        rf"(\+proj=[^\s]+\s)((?:\+[^\s]+\s)+)(\+[^\s]+){_OPTIONAL_OFFSET}",
        re.IGNORECASE,
    )
    ...
    def __eq__(self, other):
        """See ``CoordinateReferenceSystem.__eq__``."""
        return (
            isinstance(other, ProjCrs) and self._proj == other._proj and self._offset == other._offset
        )  # pylint: disable=protected-access
...
class CoordinateConverter:
    ...
    def __init__(
        self,
        from_crs: Union[CoordinateReferenceSystem, str],
        to_crs: Union[CoordinateReferenceSystem, str],
    ):
        from_crs = (
            from_crs
            if isinstance(from_crs, CoordinateReferenceSystem)
    ...
    @staticmethod
    def __get_converter(
        from_crs: CoordinateReferenceSystem, to_crs: CoordinateReferenceSystem
        ...
        # Anything dealing with a local tangent plane isn't supported by PROJ, so use pymap3d
            ...
            if isinstance(to_crs, LocalTangentPlane):
                ...
                def conv_func(points):
                    ...
                    return CoordinateConverter.__get_pyproj_func(Geodetic().get_proj_str(), to_crs.get_proj_str())(
                        np.column_stack((lon, lat, alt))
                    )
            ...
            def conv_func(points):
                points = CoordinateConverter.__get_pyproj_func(from_crs.get_proj_str(), Geodetic().get_proj_str())(
                    points
                )
